...
# Can analyze multiple time intervals by providing a range or list.
# Usage: ./scripts/analyze_win_classifier_stages.sh [options]

set -e

# Defaults (override via env or CLI)
MODEL_PATH="${MODEL_PATH:-models/win_transformer_classifier/temporal_transformer_win_classifier_best.pth}"
...
Options:
  ...
  --model_type TYPE         Backbone type: lstm | transformer | multi_task_transformer (auto if omitted)
  ...
  --test_data_path PATH     Path to single test dataset (e.g., data/splits/test_10min.parquet).
...
Examples:
  ...
  # Analyze multiple intervals using range (10,15,20,25,30,35)
  $0 --time_intervals "10-35" --model_path models/win_classifier/lstm_win_classifier_best.pth

  # Analyze specific intervals
  $0 --time_intervals "10,15,20,25,30,35" --model_path models/win_transformer_classifier/temporal_transformer_win_classifier_best.pth

  # Custom test data prefix
  $0 --time_intervals "10-30" --test_data_prefix "data/splits/test"
EOF
            exit 0 ;;
        *)
...
if [ -z "$OUTPUT_PATH" ]; then
    ...
    OUTPUT_DIR="results/Classifier_Analysis/${MODEL_NAME}"
    mkdir -p "$OUTPUT_DIR"
...
if [ -n "$TIME_INTERVALS" ]; then
    echo "=========================================="
    echo "Multi-Interval Stage Analysis"
    echo "=========================================="
