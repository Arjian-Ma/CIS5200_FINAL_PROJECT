...
`MultiStep_LSTM.py` while upgrading the architecture to match the specification:
    ...
    • 15-minute encoder ⇒ 5-minute decoder (T = 15, H = 5)
    ...
    • Decoder combines hidden-conditioned context updates with residual
      gold-difference deltas and path-aware losses (level, velocity, direction)
...
predicts residual deltas while autoregressively unfolding on past gold
...
class DynamicWeightDataset(Dataset):
    ...
    def _load(self) -> None:
        ...
        for match_id, match_df in df.groupby(self.group_column):
            ...
            previous_gold_raw = targets[enc_end - 1 : dec_end - 1]

            decoder_target_scaled = self.target_scaler.transform(decoder_target_raw.reshape(-1, 1)).squeeze(-1)
            previous_gold_scaled = self.target_scaler.transform(previous_gold_raw.reshape(-1, 1)).squeeze(-1)
            ...
            self.samples.append(
                (
                    ...
                    previous_gold_scaled.astype(np.float32),
...
class HierarchicalProjector(nn.Module):
    ...
    def predict_with_residual(self, context: Dict[str, Tensor], hidden: Tensor, prev_gold: Tensor) -> Tensor:
...
def evaluate(
    ...
    return {
        "rmse_gold_diff_scaled": rmse_scaled,
        "rmse_gold_diff_raw": rmse_raw,
...
def train(
    ...
    history = {
        "rmse_gold_diff_scaled_train": [],
        "rmse_gold_diff_raw_train": [],
        "rmse_gold_diff_scaled_val": [],
        "rmse_gold_diff_raw_val": [],
        "velocity_mae_train": [],
        "velocity_mae_val": [],
    ...
    for epoch in range(1, epochs + 1):
        ...
        scheduler.step(val_metrics["rmse_gold_diff_scaled"])
        ...
        history["rmse_gold_diff_scaled_train"].append(train_metrics_eval["rmse_gold_diff_scaled"])
        history["rmse_gold_diff_raw_train"].append(train_metrics_eval["rmse_gold_diff_raw"])
        history["rmse_gold_diff_scaled_val"].append(val_metrics["rmse_gold_diff_scaled"])
        history["rmse_gold_diff_raw_val"].append(val_metrics["rmse_gold_diff_raw"])
        ...
        current_val = val_metrics["rmse_gold_diff_scaled"]
        ...
        tqdm.write(
            f"[Epoch {epoch:02d}] "
            f"train_rmse_raw_eval={train_metrics_eval['rmse_gold_diff_raw']:.3f} | "
            f"val_rmse_raw={val_metrics['rmse_gold_diff_raw']:.3f} | "
            f"train_rmse_scaled_eval={train_metrics_eval['rmse_gold_diff_scaled']:.3f} | "
            f"val_rmse_scaled={val_metrics['rmse_gold_diff_scaled']:.3f} | "
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=str, default="results/seq2seq_dynamic", help="Output directory.")
...
def main(args: argparse.Namespace) -> None:
    ...
    model_path = output_dir / "seq2seq_dynamic_weight.pth"
    torch.save(
        {
    ...
    print(
        f"[8/8] Validation performance → rmse_raw={metrics['rmse_gold_diff_raw']:.3f}, "
        f"rmse_scaled={metrics['rmse_gold_diff_scaled']:.3f}, "
