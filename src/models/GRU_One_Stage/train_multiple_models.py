"""
Train multiple GRU One Stage models with different time windows (10, 15, 20, 25, 30 minutes).
...
model_path = Path(__file__).parent / "GRU_One_Stage.py"
spec = importlib.util.spec_from_file_location("gru_one_stage", model_path)
gru_module = importlib.util.module_from_spec(spec)
sys.modules[spec.name] = gru_module
spec.loader.exec_module(gru_module)

HierarchicalWinPredictor = gru_module.HierarchicalWinPredictor
WinPredictionDataset = gru_module.WinPredictionDataset
collate_fn = gru_module.collate_fn
ALL_FEATURE_COLUMNS = gru_module.ALL_FEATURE_COLUMNS
TARGET_COLUMN = gru_module.TARGET_COLUMN
GROUP_COLUMN = gru_module.GROUP_COLUMN
TIME_COLUMN = gru_module.TIME_COLUMN
...
OUTPUT_DIR = ROOT / "results/gru_one_stage_multiple"
...
def train_model(
    ...
    """Train a single GRU model with specified encoder length."""
    print(f"\n{'=' * 60}")
    print(f"Training GRU Model: {encoder_length} minutes")
    print(f"{'=' * 60}")
...
def main():
    print("=" * 60)
    print("TRAINING MULTIPLE GRU ONE STAGE MODELS")
    print("=" * 60)
