#!/usr/bin/env python3
"""
Random Forest Classifier for League of Legends match prediction
...
OUTPUT_DIR = "results/Model_RandomForest_Classification"

# Create output directory
os.makedirs(OUTPUT_DIR, exist_ok=True)

print("\n" + "="*60)
print("RANDOM FOREST CLASSIFIER - WIN PREDICTION")
print("="*60)
...
rf_model = RandomForestClassifier(
    ...
    min_samples_split=2,
    min_samples_leaf=1,
    max_features='sqrt',  # Random Forest uses feature subsampling
    bootstrap=True,
    random_state=42,
    verbose=0,
...
axes[0].set_title(f'ROC Curve - Test Set (Frame {PREDICTION_FRAME_IDX}) - Random Forest', fontsize=14, fontweight='bold')
...
axes[1].set_title(f'Confusion Matrix - Test Set (Frame {PREDICTION_FRAME_IDX}) - Random Forest', fontsize=14, fontweight='bold')
axes[1].set_xticklabels(['Lose (0)', 'Win (1)'])
axes[1].set_yticklabels(['Lose (0)', 'Win (1)'])

plt.tight_layout()
plt.savefig(f'{OUTPUT_DIR}/random_forest_classification_results_frame{PREDICTION_FRAME_IDX}.png', 
            dpi=300, bbox_inches='tight')
print(f"✓ Visualization saved as '{OUTPUT_DIR}/random_forest_classification_results_frame{PREDICTION_FRAME_IDX}.png'")
